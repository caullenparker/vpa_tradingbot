...
st.set_page_config(page_title="Crescent Lake Capital | VPA TradingBot", layout="wide")
...
def apply_theme():
    ...
    st.markdown(
        ...
        <style>
            :root {{
                --clc-bg: {t["bg"]};
                --clc-bg-secondary: {t["bg_secondary"]};
                --clc-sidebar: {t["sidebar"]};
                --clc-card: {t["card"]};
                --clc-card-alt: {t["card_alt"]};
                --clc-text: {t["text"]};
                --clc-muted: {t["muted"]};
                --clc-border: {t["border"]};
                --clc-accent: {t["accent"]};
                --clc-accent-soft: {t["accent_soft"]};
                --clc-accent-light: {t["accent_light"]};
                --clc-success: {t["success"]};
                --clc-warning: {t["warning"]};
                --clc-danger: {t["danger"]};
                --clc-input: {t["input"]};
            }}
            ...
            .stApp {{
                background:
                    radial-gradient(circle at 50% 0%, color-mix(in srgb, var(--clc-accent-soft) 14%, transparent), transparent 28%),
                    var(--clc-bg);
                color: var(--clc-text);
            }}
            ...
            div[data-baseweb="select"] > div {{
                background: var(--clc-input) !important;
                color: var(--clc-text) !important;
                border-color: var(--clc-border) !important;
            }}
            ...
            .clc-login-shell {{
                ...
                background: color-mix(in srgb, var(--clc-card) 94%, transparent);
                border: 1px solid var(--clc-border);
                border-radius: 20px;
...
def render_branding(compact=False):
    width = 130 if compact else 260
    st.markdown(
        f"""
        <div class="clc-brand">
            <img src="data:image/png;base64,{CLC_LOGO_BASE64}" style="width:{width}px;">
            <div class="clc-brand-name">VPA TRADINGBOT</div>
            <div class="clc-brand-subtitle">A Crescent Lake Capital Platform</div>
        </div>
        """,
        unsafe_allow_html=True
    )
...
def login_gate():
    ...
    theme_col1, theme_col2, theme_col3 = st.columns([1, 0.6, 1])
    with theme_col2:
        selected_theme = st.segmented_control(
            "Appearance",
            options=["Dark", "Light"],
            default=st.session_state.theme_mode,
            key="login_theme_control",
    ...
    with center:
        st.markdown('<div class="clc-login-shell">', unsafe_allow_html=True)
        st.markdown(
            f"""
            <img src="data:image/png;base64,{CLC_LOGO_BASE64}" class="clc-login-logo">
            <div class="clc-login-title">VPA TradingBot</div>
            <div class="clc-login-caption">Secure access by Crescent Lake Capital</div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            <div class="clc-footer">
                Discipline · Perspective · Compounding<br>
                Educational only · Not financial advice
            </div>
            </div>
            """,
...
with st.sidebar:
    render_branding(compact=True)

    selected_theme = st.segmented_control(
        "Appearance",
        options=["Dark", "Light"],
        default=st.session_state.theme_mode,
        key="sidebar_theme_control",
        label_visibility="collapsed"
    )
